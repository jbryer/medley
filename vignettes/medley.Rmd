...
model_sum <- data.frame(
  Model = seq_len(medley_lr_out$n_models),
  n = apply(medley_lr_out$model_observation, 2, sum),
  Retained = apply(medley_lr_out$model_observation, 2, FUN = function(x) {
    daacs_train[x,]$retained |> mean() * 100
  }),
  `Model Formula` = as.character(medley_lr_out$formulas),
  check.names = FALSE
)
```
