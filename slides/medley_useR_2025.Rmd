...
subtitle: "useR! 2025, Duke University"
...
4. The `medley` approach to predictive modeling.

5. Compare the results across the various modeling approaches.

<br/><br/>

.font70[This research was developed under grant [R305A210269](https://ies.ed.gov/use-work/awards/examining-efficacy-predictive-power-and-cost-effectiveness-diagnostic-assessment-and-achievement?ID=4549) from the U.S.
...
The Diagnostic Assessment and Achievement of College Skills (DAACS; [www.daacs.net](https://daacs.net)) is a suite of technological and social supports designed to optimize student learning. It asseses college readiness in a *no-stakes* environment.
...
One of our primary research question is whether **can DAACS improve the accuracy of predicting student success** above what institutions already know?

Problem is that although students were expected to complete DAACS as part of orientation, many students did not attend orientation, and then some students did not complete all four assessments.
...
```{r, echo=FALSE, out.height='500px', out.width='1000px', fig.align='center'}
knitr::include_graphics('images/DAACS_Retained.png')
```

.font70[See [Bryer, Andrade, Cleary, Lui, Franklin, & Akhmedjanova, (in press)](https://github.com/daacs/FIPSE-Efficacy) for more details.]
...
For methods 1, 2, and 4 we will use both logistic regression and random forests with each of these approaches. Why random forest? See [Fernandez-Delgado, Cernadas, Barro, and Amorim (2014)](https://jmlr.org/papers/volume15/delgado14a/delgado14a.pdf).
...
First we need to subset our training data frame with variables where there are no missing values.

```{r demographics-model}
missingness <- apply(daacs_train, 2, FUN = function(x) { sum(is.na(x)) / length(x)})
daacs_train_complete <- daacs_train[,names(missingness)[missingness == 0]]
names(daacs_train_complete)
```

We will train a model using logistic regression...

```{r}
lr_out <- glm(data = daacs_train_complete,
              formula = retained ~ .,
...
```{r}
rf_out <- randomForest(formula = factor(retained) ~ .,
...
```{r}
daacs_complete_mean <- daacs                                            # Create a copy of the data.frame
for(i in 2:ncol(daacs_complete_mean)) {                                 # Loop through all the variables,
	missing_rows <- is.na(daacs_complete_mean[,i])                      # find the rows with missing
	if(sum(missing_rows) > 0) {                                         # values and replace with mean.
		daacs_complete_mean[missing_rows, i] <- mean(daacs_complete_mean[,i], na.rm = TRUE)
	}
}

daacs_train_complete_mean <- daacs_complete_mean[train_rows,]           # Split into training and
daacs_valid_complete_mean <- daacs_complete_mean[-train_rows,]          # validation data.frames

mean_lr_out <- glm(formula = retained ~ .,                              # Logistic regression
                   data = daacs_train_complete_mean,
                   family = binomial(link = logit))
mean_lr_predictions <- predict(mean_lr_out,                             # Random forest
							   newdata = daacs_valid_complete_mean, 
							   type = 'response')

mean_rf_out <- randomForest(formula = factor(retained) ~ .,             # Logistic regression predictions
                            data = daacs_train_complete_mean)
mean_rf_predictions <- predict(mean_rf_out,                             # Random forest predictions
							   newdata = daacs_valid_complete_mean, 
							   type = 'response')
```
...
.pull-left[
Parameters for the `medley()` function:
...
* `min_set_size` - The minimum (as a percentage) size of any dataset to train a model. The default is 10%.
* `exclusive_membership` - If `TRUE`, any observation will be used in exactly one model.
* `...` - Other parameters passed to the `method` function. Default is `FALSE`.
]
...
---
# Medley Trainig and Predicting

.pull-left[
#### Training

You can use `medley` like most modeling functions in R (i.e. there are `data` and `formula` parameters).


```{r}
medley_lr_out <- medley(
	data = daacs_train,
	formula = retained ~ .,
	method = glm,
	family = binomial(link = logit))
...
Also like most modeling functions, the `predict()` function works as expected.

```{r}
medley_lr_predictions <- predict(
	medley_lr_out,
	newdata = daacs_valid,
	type = 'response')
```
...
knitr::kable(overall, 
  ...
  kableExtra::add_footnote(
    paste0('Improvement is the difference with the overall base retention rate of ', 
...
* Would like to allow for using different models within a single training/prediction problem.

### Miscellaneous

* [Downsampling](https://jbryer.github.io/medley/articles/downsampling.html) - the `downsampling()` function is designed for situations where there is significant imbalance in you dependent variable.

* [`recode_small_levels()`](https://jbryer.github.io/medley/reference/recode_small_levels.html) is a utility function that will find rarely used factor levels and recode them.
